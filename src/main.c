...
char* select_state_buffer = NULL;
uint16_t* char_buffer = NULL;
...
void calculator_run(struct nk_context *ctx) {
  ...
  if (nk_begin(ctx, "Calculator", nk_rect(0, 0, window_width, window_height), 0x00))
  {
    ...
    if (clicked_active || rclicked_active) {
      ...
      select_state_buffer[y * appState.col_w + x] = clicked_active? 1: 0;
    }
    ...
    if (nk_tree_push(ctx, NK_TREE_NODE, "Edit", NK_MAXIMIZED)) {
      ...
      if (nk_button_symbol(ctx, NK_SYMBOL_TRIANGLE_UP)) {
        MovePixelsUp(select_state_buffer, appState.col_w, appState.col_h);
      }
      nk_label(ctx, "", NK_TEXT_LEFT);

      if (nk_button_symbol(ctx, NK_SYMBOL_TRIANGLE_LEFT)) {
        MovePixelsLeft(select_state_buffer, appState.col_w, appState.col_h);
      }
      nk_label(ctx, "", NK_TEXT_LEFT);
      if (nk_button_symbol(ctx, NK_SYMBOL_TRIANGLE_RIGHT)) {
        MovePixelsRight(select_state_buffer, appState.col_w, appState.col_h);
      }

      nk_label(ctx, "", NK_TEXT_LEFT);
      if (nk_button_symbol(ctx, NK_SYMBOL_TRIANGLE_DOWN)) {
        MovePixelsDown(select_state_buffer, appState.col_w, appState.col_h);
      }
      nk_label(ctx, "", NK_TEXT_LEFT);
...
void parse_clipboard() {
  ...
  if (count != appState.col_h) {
    memset(select_state_buffer, 0, appState.col_w * appState.col_h);
    memset(char_buffer, 0, sizeof(uint16_t) * appState.col_h);
    // show error
    return;
  }
  load_char_buffer();
...
void create_char_buffer() {
  ...
  for (i = 0; i < appState.col_h; i++)
  {
    ...
    {
      int bit = 15 - j;
      if (select_state_buffer[j + (i * appState.col_w)])
      {
...
void load_char_buffer() {
  ...
  for (i = 0; i < appState.col_h; i++)
  {
    ...
    for (j = 0; j < appState.col_w; j++)
    {
      if ((b << j) & 0x8000)
      {
        select_state_buffer[j + (i * appState.col_w)] |= 1 << (i % 8);
      }
      else
      {
        select_state_buffer[j + (i * appState.col_w)] &= ~(1 << (i % 8));
...
void free_buffers() {
  if (char_buffer)
    free(char_buffer);
  if (select_state_buffer)
    free(select_state_buffer);
}

void create_buffers() {
  free_buffers();

  select_state_buffer = malloc(appState.col_w * appState.col_h);
  memset(select_state_buffer, 0, appState.col_w * appState.col_h);

  char_buffer = malloc(sizeof(uint16_t) * appState.col_h);
  memset(char_buffer, 0, sizeof(uint16_t) * appState.col_h);
}
